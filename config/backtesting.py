    ...
    for symbol in stock_assets[:50]:
        try:
            ...
            if price_change_24h is not None and price_change_24h < 15:
                score += 1
            if volume and volume_7d_avg and volume > 0.9 * volume_7d_avg:
                score += 1

            if score >= MIN_CRITERIA:
                if not is_approved_by_finnhub_and_alphavantage(symbol):
                    print(f"⛔ {symbol} no aprobado por filtros externos")
                    continue
